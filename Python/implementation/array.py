...
class Array:
    ...
    def insert(self, element, index: Optional[int] = None) -> List:
        ...
        Args:
            ...
            index (Optional[int], optional): Index to insert the element at. Use this parameter
            only if you want to displace an already initialized element. Defaults to None.

        Raises:
            TypeError: This is raised when the element doesn't match what the array was 
            initialized with.
            ValueError: This is raised when the index specified defies the contiguous logic of
            and array.
        ...
        if not isinstance(element, self.__data_type):
            raise TypeError(
                f"Arrays should be homologous, this value is not of {self.__data_type} type."
                )
        if self.__counter >= self.__length_of_array:
            self.resize()
        if index is not None:
            if self.check_index_validity(index):
                for index_difference, value in enumerate(self.__array[self.__counter-1:index-1:-1]):
                    self.__array[self.__counter-index_difference] = value
                self.__array[index] = element
                self.__counter+=1
            else:
                raise ValueError(
                    "Arrays should be contiguous. The index is not a valid point to put an element."
                    )
        else:
    ...
    def __getitem__(self, index: int):
        """
        Function that allows you to access elements using the square braces and an index.
    ...
    def delete(self, index: int) -> None:
        ...
        for index_difference, value in enumerate(self.__array[index+1: self.__counter]):
